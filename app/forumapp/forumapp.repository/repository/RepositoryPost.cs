...
namespace forumapp.repository.repository
{
    public class RepositoryPost : RepositoryBase<dbPost>, IRepositoryPost
    {
        ...
        public async Task<ICollection<dbPost>> FindAllByPost(int post)
        {
            try
            {
                return await Task.FromResult(lsPost.Where(x => x.Id == post || x.Reply == post).ToList());
            }
            catch (Exception) { throw; }
        }
        ...
        public override async Task<dbPost> Insert(dbPost item)
        {
            try
            {
                if (item.Id != 0)
                {
                    var existe = lsPost.Where(x => x.Id == item.Id).FirstOrDefault();

                    existe.Comment = item.Comment;
                }
                else
                {
        ...
        public override async Task<int> Delete(int id)
        {
            try
            {
                var existe = lsPost.Where(x => x.Id == id).FirstOrDefault();

                if (existe != null)
                {
                    if (existe.Reply == 0)
                        lsPost.RemoveAll(x => x.Id == id || x.Reply == id);
                    else
                        lsPost.RemoveAll(x => x.Id == id);
